function sh_mask = shadowdem(I, res, azi, zen)
...
% Low limit value defining when there is no data below this value
NaN_LL = -32000;
% Top limit value defining when there is no data above this value
NaN_TL = 65000;
...
% rescale indices
[y_ind, x_ind] = ind2sub([ylim, xlim], xy_ind);
...
% check if angles are given as scalars or arrays
if isscalar(azi) && isscalar(zen)
    % check if the angle is in the areas where sine of azimuth is bigger
    % than the cosine meaning that shifts in y axis are bigger
    if ((azi>pi*0.25)&&(azi<pi*0.75))||((azi>pi*1.25)&&(azi<pi*1.75))

        % check the direction of the shift
        if gt(y_aux, 0)
            for j = 1:n_elev
                % update when the number of steps reaches next step in
                % waitbar
                if j >= upd_count * upd_step
                    ...
                    % output) sec2str() function: convert numbers to
                    % strings, dividing into minutes, hours and days if
                    % necessary ceil(upd_count * upd_rate) - percent,
                ...
                if (~sh_mask(y_top,x_top))&&(z_top>NaN_LL)&&(z_top<NaN_TL)
                    ...
                    % every x coordinate may have two or more
                    % corresponding y coordinates (in other words, two
                    % different y coordinates may point to the same x
                    % coordinate)
                    for k = 2:y_n
                        y_t1 = y_p(k);
                        y_t2 = y_p(k+1);
                        
                        % calculate two x coordiantes assigned for the two
                        % y coordinates
                        x_c1 = round(x_top + cot_azi * (y_top - y_t1));
                        x_c2 = round(x_top + cot_azi * (y_top - y_t2));
                        
                        % check whether the points are within the image
                        % limits
                        if (x_c1>0)&&(x_c1<=xlim)&&(y_t1>0)&&...
                                (y_t1<=ylim)
                            
                            % ratio for the current pair of coordinates
                            t = (y_top - y_t1) / y_d;
                            
                            % calculate the height of the line in the
                            % current coordinates pair
                            h_c = h * t + z_top;
                            
                            % compare the height of the line in the
                            % current point with the height at this point
                            % in the image
                            if h_c > I(y_t1, x_c1)
                                
                                % if the line is above the terrain then
                                % the shadow is cast from the top point
                                % towards the current pair of coordinates
                                % (point)
                                sh_mask(y_t1, x_c1) = true;
                                
                                % if the line is not above the terrain at
                                % the current point then check the
                                % neighbouring pixel whether two different
                                % y coordinates point to the same x
                                % coordinate
                            elseif (x_c1==x_c2)&&(y_t2>0)&&(y_t2<=ylim)
                                if h_c > I(y_t2, x_c1)
            ...
            for j = 1:n_elev
                ...
                if (~sh_mask(y_top,x_top))&&(z_top>NaN_LL)&&(z_top<NaN_TL)
                    ...
                    for k = 2:y_n
                        ...
                        x_c1 = round(x_top + cot_azi * (y_top - y_t1));
                        x_c2 = round(x_top + cot_azi * (y_top - y_t2));
                        if (x_c1>0)&&(x_c1<=xlim)&&(y_t1>0)&&(y_t1<=ylim)
                            t = (y_top - y_t1) / y_d;
                            h_c = h * t + z_top;
                            if h_c > I(y_t1, x_c1)
                                sh_mask(y_t1, x_c1) = true;
        ...
        % cosine of azimuth is bigger than sine (shifts in x axis are
        % bigger)
        if gt(x_aux, 0) % check the direction of the shift
            for j = 1:n_elev
                if j >= upd_count * upd_step
                    t_elp = toc;
                    fprintf(repmat('\b', 1, numel(out_str)));
                ...
                if (~sh_mask(y_top,x_top))&&(z_top>NaN_LL)&&(z_top<NaN_TL)
                    ...
                    for k = 2:x_n
                        ...
                        if (x_t1>0)&&(x_t1<=xlim)&&(y_c1>0)&&(y_c1<=ylim)
                            t = (x_top - x_t1) / x_d;
                            h_c = h * t + z_top;
                            if h_c > I(y_c1, x_t1)
                                sh_mask(y_c1, x_t1) = true;
                            elseif (y_c1==y_c2)&&(x_t2>0)&&(x_t2<=xlim)
                                if h_c > I(y_c1, x_t2)
            ...
            for j = 1:n_elev
                ...
                end % validate for non-shadow meaningful pixels
            end % loop through all peaks
        end % the x direction shift
    end % choose the azimuth sector
    
else % the angles are given as arrays
    % auxiliary angles to choose the azimuth sector
    pi_aux1 = pi * 0.25; pi_aux2 = pi * 0.75;
    pi_aux3 = pi * 1.25; pi_aux4 = pi * 1.75;
    for j = 1:n_elev
        ...
        if (~sh_mask(y_top, x_top))&&(z_top > NaN_LL)&&(z_top < NaN_TL)
            ...
            azi_aux = azi(y_top, x_top);
            % check if the angle is in the areas where sine of azimuth is
            % bigger than the cosine meaning that shifts in y axis are
            % bigger
                        
            if ((azi_aux > pi_aux1) && (azi_aux < pi_aux2))||...
                    ((azi_aux>pi_aux3) && (azi_aux < pi_aux4))
                
                % calculate the shift in y direction
                y_d = h * y_aux(y_top, x_top);
                % check the direction of the shift
                if y_aux(y_top, x_top) > 0
                    y_n = ceil(y_d);
                    y_p = y_top:y_top + y_n;
                else
                    y_n = ceil(-y_d);
                ...
                for k = 2:y_n
                    ...
                    % coordinates
                    x_c1 = round(x_top + cot_aux * (y_top - y_t1));
                    
                    % check whether the points are within the image limits
                    if (x_c1>0)&&(x_c1<=xlim) && (y_t1>0)&&(y_t1<=ylim)
                        
                        % use the ratio for the current pair of
                        % coordinates to calculate the height of the line
                        h_c = h * (y_top - y_t1) / y_d + z_top;
                        % neighbouring pixel
                        y_t2 = y_p(k+1);
                        ...
                        elseif (x_c1 == round(x_top+cot_aux*(y_top - ...
                                y_t2))) && (y_t2 > 0) && (y_t2 <= ylim)
                            if h_c > I(y_t2, x_c1)
                                sh_mask(y_t2, x_c1) = true;
                            else
                                break;
                ...
                x_d = -h * x_aux(y_top, x_top);
                
                if x_aux(y_top, x_top) > 0
                    x_n = ceil(-x_d);
                    x_p = x_top:-1:x_top - x_n;
                else
                    x_n = ceil(x_d);
                ...
                tan_aux = tan_azi(y_top, x_top);
                for k = 2:x_n
                    x_t1 = x_p(k);
                    x_t2 = x_p(k+1);
                    y_c1 = round(y_top + tan_aux * (x_top - x_t1));
                    y_c2 = round(y_top + tan_aux * (x_top - x_t2));
                    if (x_t1>0)&&(x_t1<=xlim) && (y_c1>0)&&(y_c1<=ylim)
                        h_c = h * (x_top - x_t1) / x_d + z_top;
                        if h_c > I(y_c1, x_t1)
                            sh_mask(y_c1, x_t1) = true;
                        elseif (y_c1==y_c2) && (x_t2>0) && (x_t2<=xlim)
                            if h_c > I(y_c1, x_t2)
...
end % checking if angles were given as scalars or arrays

fprintf('\n');

end % end main function (shadowdem)