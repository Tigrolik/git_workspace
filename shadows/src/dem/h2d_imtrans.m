%H2D_IMTRANS - Homogenous transformation of an image.
% (Old EVEX_IMTRANS.M)
%
% newim = h2d_imtrans(img, T)
% newim = h2d_imtrans(img, T, 'parameter', value...)
% [newim, newT] = h2d_imtrans(...)
%
...
%
% See also: evex_affinemat, evex_pttrans
%
%
...
% $Id: h2d_imtrans.m,v 1.1 2008/07/21 07:35:27 jkamarai Exp $

function [newim, newT] = h2d_imtrans(im, T, varargin);
...
sxy = h2d_trans([xi(:)'+B(3) ; yi(:)'+B(1) ; ones(1,ncols*nrows)],Tinv);
