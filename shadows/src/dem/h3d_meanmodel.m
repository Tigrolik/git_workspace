%H3D_MEANMODEL Mean model of K point sets <STILL EXPERIMENTAL BY JONI>
%
% [Xm,mgd,Xt] = h3d_meanmodel(X_,tr_,:)
%
% For 3-D points patterns in X_ compute their mean model
% under the selected 3-D homography tr_. In principle, the function
% works exactly as H2D_MEANMODEL.M, but in 3D.
%
% Output:
...
%     0:   No transformation
%     1:   Isometry (hnd_corresp_umeyama.m)
%     2:   Similarity (hnd_corresp_umeyama.m)
...
%
function [bestXm,bestMgd,bestXt,varargout] = h3d_meanmodel(X_,tr_,varargin)
...
for randInd = 1:conf.randRepeats
  ...
  for pointInd = 1:size(X_,1)
    ...
    geomDists(pointInd) = sum(sqrt(sum((X'-hnd_trans(Xm',inv(H))').^2,2)));
  end;
