...
public:
    ...
    template <class Comp>
    Mat& operator*=(const Comp&);
    //multiply by vector element-wise or matrix multiplication???
    //template <class Comp>
    //Mat& operator*=(const Vec<R, Comp>&);
    template <class Comp>
    Mat& operator/=(const Comp&);
...
template <size_t R, size_t C, class Num>
inline Vec<R, Num> Mat<R, C, Num>::col(const size_t i) {
    Vec<R, Num> v;
    for (auto j = R; j--; v[j] = M_[j][i]) { }
    return v;
}

template <size_t R, size_t C, class Num>
inline const Vec<R, Num> Mat<R, C, Num>::col(const size_t i) const {
    Vec<R, Num> v;
    for (auto j = R; j--; v[j] = M_[j][i]) { }
    return v;
}
...
// multiply every matrix column by a vector (element-wise) ???
//template <size_t R, size_t C, class Num> template <class Comp>
//inline Mat<R, C, Num>& Mat<R, C, Num>::operator*=(const Vec<R, Comp> &rhs) {
//    for (auto j = C; j--;)
//        for (auto i = R; i--; M_[i][j] *= rhs[i]) { }
...
// divide every matrix column by a vector (element-wise) ???
//template <size_t R, size_t C, class Num> template <class Comp>
//inline Mat<R, C, Num>& Mat<R, C, Num>::operator/=(const Vec<R, Comp> &rhs) {
//    for (auto j = C; j--;)
//        for (auto i = R; i--; M_[i][j] /= rhs[i]) { }
