...
XMARGIN = int((WINDOWWIDTH -
               (TILESIZE * BOARDWIDTH + (BOARDWIDTH - 1))) / 2)
...
def main():  # NOQA
    ...
    pygame.display.set_caption('Slize Puzzle')
    ...
    # solved board is the same as the board in the start state
    ...
    while True:  # main game loop
        ...
        slideTo = None
        # message to display in the top left corner
            ...
            if event.type == MOUSEBUTTONUP:
                ...
                if (spotx, spoty) == (None, None):
                    ...
                    if RESET_RECT.collidepoint(event.pos):
                        ...
                        # clicked on the Solve button
                        resetAnimation(mainBoard, solutionSeq + allMoves)
                        allMoves = []
                else:
                    # check if the clicked time was next to the blank spot
        ...
        if slideTo:
            ...
            slideAnimation(mainBoard, slideTo, 'Click tile or press arrow keys'
                           'to slide.', 8)
...
def getStartingBoard():
    ...
    # For example, if BOARDWIDTH and BOARDHEIGHT are both 3, then return
...
def getBlankPosition(board):
    # return x and y board coordinates of the blank space
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT):
            if board[x][y] == BLANK:
                return (x, y)
...
def getSpotClicked(board, x, y):
    # from x and y pixel coordinates get x and y board coordinates
...
def drawTile(tilex, tiley, number, adjx=0, adjy=0):
    # draw a tile at board coordinates tilex and tiley optionally with a few
    ...
    textRect.center = left + int(TILESIZE / 2) + adjx, \
...
def generateNewPuzzle(numSlides):
    # from a starting configuration make numSlides of moves and animate them
    sequence = []
    ...
    for i in range(numSlides):
        ...
        slideAnimation(board, move, 'Generatin new puzzle...',
...
def resetAnimation(board, allMoves):
    # make all moves in allMoves in reverse
